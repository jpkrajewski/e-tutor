...
def get_students_missing_payments(queryset):
    return (
        queryset.filter(payment__status=Payment.DUE)
...
def get_student_missing_payment(queryset):
    return queryset.filter(status=Payment.DUE).annotate(missing_payment=Sum("amount"))


def get_total_student_missing_payment(queryset) -> int:
    return queryset.filter(status=Payment.DUE).aggregate(missing_payment=Sum("amount"))[
        "missing_payment"
    ]
...
def get_lessons_today_and_tomorrow(queryset):
    ...
    return queryset.filter(
        start_datetime__range=(start_of_a_day, end_of_a_tomorrow)
    ).filter(start_datetime__gte=datetime.now(tz=timezone.utc))
...
def get_money_per_week(queryset) -> int:
    ...
    return queryset.filter(start_datetime__range=(start, end)).aggregate(
        money_weekly=Sum("amount")
    )["money_weekly"]
