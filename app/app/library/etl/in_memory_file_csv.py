...
class InMemoryFileCSVHandler:
    ...
    def __init__(
        self, csv_file, model_in_csv, uploader_model, msg_fail=None, msg_success=None
    ) -> None:
        self._csv = csv_file
        ...
        self._msg_success = (
            "{} loaded to data base." if not msg_success else msg_success
        )
        ...
        for mapped_obj in self._data:
            if self._filter_func(mapped_obj):
                self._log.append(
                    {
                        "status": "fail",
                        "msg": self._msg_fail.format(self._model(**mapped_obj)),
                    }
                )
                continue
    ...
    def _load(self):
        for mapped_obj in self._data:
            ...
            self._log.append(
                {"status": "success", "msg": self._msg_success.format(added_obj)}
            )
...
class InMemoryStudentCSVHandler(InMemoryFileCSVHandler):
    def _filter_func(self, student):
        for arg in ["facebook_profile", "discord_nick", "email", "phone_number"]:
            if (
                self._model.objects.filter(**{arg: student[arg]})
                .exclude(**{arg: ""})
                .exists()
            ):
                return True
