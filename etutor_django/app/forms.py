    ...
    def clean(self):
        ...
        if (
            Student.objects.filter(
                tutor=self.tutor, phone_number=cleaned_data["phone_number"]
            ).exists()
        ...
        if (
            Student.objects.filter(
                tutor=self.tutor, discord_nick=cleaned_data["discord_nick"]
            ).exists()
        ...
        if (
            Student.objects.filter(
                tutor=self.tutor, email=cleaned_data["email"]
            ).exists()
...
class StudentCreateFromCSVForm(forms.Form):
    csv_with_students = forms.FileField(
